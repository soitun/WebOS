<cfset current_box ="newsletters.cfm">
...
<cfif isDefined('session.user_id') and session.user_id NEQ "0">
  <!---\\ We're now goin to check to see if this user signed up for the newsletter when they registered, or updated thier profile//--->
   ...
   <cfquery name="is_subscribed" datasource="#datasrc#">
    Select * from #table_usrs# where username='#session.username#' AND SiteID = <cfqueryparam cfsqltype="cf_sql_char" value="#session.SiteID#">
   </cfquery> 
...
Not a Member of Our Site? <a href="http://<cfoutput>#site_domain##site_folder#</cfoutput>index.cfm?module=users&section=register">Click Here</a>
</td>
</tr>
</table>
</cfif>
