<div id="menu_box_title">Our Newsletter</div>
<div id="menu_box">
...
<cfif isDefined('session.user_id') and session.user_id NEQ "0">
  <!---\\ We're now goin to check to see if this user signed up for the newsletter when they registered, or updated thier profile//--->
   <cfset user = application.user.getUser(session.username)>
   <cfset request.user = user>
   <cfset session.user = user>
   <cfset session.user.session_key = session.session_key>
   ...
   <cfquery name="is_subscribed" datasource="#datasrc#">
    Select * from #table_usrs# where id='#session.user_id#'
   </cfquery> 
